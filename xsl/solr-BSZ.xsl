...
<!--
	Post-processes solr records coming from the BSZ BOSS server.	
	
	2012        Sven-S. Porst, SUB Göttingen <porst@sub.uni-goettingen.de>
	2014-2015:  modified by Clemens Elmlinger, BSZ Konstanz <clemens.elmlinger@bsz-bw.de>
		        (BSZ intermediate language, ISBN10/13,...)
		...
		<pz:metadata type="medium">
			<xsl:choose>
				...
				<xsl:when test=".='Video' or .='VHS'">video</xsl:when>
                                ...
                                <xsl:otherwise>
                                  <xsl:if test="../pz:metadata[@type='content']='gkko'">proceeding</xsl:if>
                                </xsl:otherwise>
